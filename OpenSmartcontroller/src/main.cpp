...
{ // put your main code here, to run repeatedly:
    ...
    if (tp.z > MINPRESSURE && tp.z < MAXPRESSURE)
    {
        ...
        if ((tp.x > plusAmb.x1) && tp.x < plusAmb.x2)
        {                                                 //Amb + OR -ve touched
            if (tp.y <= plusAmb.y1 && tp.y >= plusAmb.y2) //Was it the + ?
                ambTempTh++;
            if (tp.y <= minusAmb.y1 && tp.y >= minusAmb.y2)
            {                      //Was it the - ?
        ...
        if ((tp.x > plusPipe.x1) && tp.x < plusPipe.x2)
        {                                                   //PipeTemp + OR -ve touched
            if (tp.y <= plusPipe.y1 && tp.y >= plusPipe.y2) //Was it the + ?
                pipeTempTh++;
            if (tp.y <= minusPipe.y1 && tp.y >= minusPipe.y2)
            { //Was it the - ?
                pipeTempTh--;
            }
        }
        ...
        */
        if ((tp.x > fanOnButton.x1) && tp.x < fanOnButton.x2)
        { //In the button regions?
            if (tp.y <= fanOnButton.y1 && tp.y >= fanOnButton.y2)
            { //Was it the FAN ON button  ?
            ...
            if (tp.y <= setButton.y1 && tp.y >= setButton.y2)
            { //Was it the set button ?
                SendPacket(ambTempTh, pipeTempTh, fanOn, fanOff);
            }
        }
        ...
        if ((tp.x > setLow.x1) && tp.x < setLow.x2) //In the button regions?
            if (tp.y <= setLow.y1 && tp.y >= setLow.y2)
            { //Was it the set low temp button  ?
                if (ambTempTh > 9)
                    tft.fillRect(30, 177, 100, 20, BLACK);
                ambTempTh = 7;
                //tft.fillRoundRect(295,120,100,60,8,VGA_AQUA);
